...
/-- The displacement `f x - x` of a function on the positive reals. -/
def displacement (f : PositiveReal → PositiveReal) (x : PositiveReal) : ℝ :=
  (f x : ℝ) - (x : ℝ)

/-- Squared form of the left inequality: `(f(x) + y)^2 ≤ 2 (x^2 + f(y)^2)`. -/
lemma admissible_sq_upper {f : PositiveReal → PositiveReal} (hf : IsAdmissible f)
    (x y : PositiveReal) :
...
lemma admissible_sq_lower {f : PositiveReal → PositiveReal} (hf : IsAdmissible f)
    (x y : PositiveReal) :
...
/-- Iterating `f` twice shifts by the displacement: `f(f(y)) = 2 f(y) - y`. -/
...
/-- The displacement is constant along the orbit of `f`. -/
lemma admissible_displacement_f {f : PositiveReal → PositiveReal} (hf : IsAdmissible f)
    (y : PositiveReal) : displacement f (f y) = displacement f y := by
  rw [displacement, displacement, admissible_orbit hf]
  ring

/-- The displacement is constant along every finite orbit of `f`. -/
lemma admissible_displacement_iterate {f : PositiveReal → PositiveReal}
...
/-- The orbit of `f` is an arithmetic progression with difference
`displacement f y`. -/
lemma admissible_iterate_coe {f : PositiveReal → PositiveReal} (hf : IsAdmissible f)
    (y : PositiveReal) (n : ℕ) :
...
/-- The displacement is nonnegative: `f y ≥ y`. -/
lemma admissible_displacement_nonneg {f : PositiveReal → PositiveReal}
...
/-- All positive values of the displacement coincide. -/
lemma admissible_positive_displacements_eq {f : PositiveReal → PositiveReal}
...
/-- A zero of the displacement is separated by a definite distance from any
point where the displacement takes a positive value. -/
lemma admissible_zero_positive_separated {f : PositiveReal → PositiveReal}
...
/-- The displacement is constant: it takes at most the two values `0` and
`c > 0`, it is locally constant on the preconnected space `ℝ_{>0}`, and hence
globally constant. -/
lemma admissible_displacements_eq {f : PositiveReal → PositiveReal}
    (hf : IsAdmissible f) (x y : PositiveReal) :
    displacement f x = displacement f y := by
