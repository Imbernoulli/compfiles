...
/-- The predicate stating that `a : ℕ → ℕ` (0-indexed) is a sequence satisfying
the problem's condition: each term exceeds `1`, and each subsequent term is the
smallest integer strictly larger than the previous one that shares a common
factor with every earlier term. -/
...
/-- If the prime factors of `x` are contained in those of `y`, then any `b`
sharing a common factor with `x` also shares one with `y`. -/
lemma not_coprime_of_primeFactors_subset {b x y : ℕ}
    (hx : x ≠ 0) (_hy : y ≠ 0) (hxy : x.primeFactors ⊆ y.primeFactors)
...
/-- If the radical `∏ p ∈ x.primeFactors, p` of `x` exceeds `A ^ (A + 2)` and
`q` is a prime factor of `x`, then there is a `b ∈ [A, rad x)` whose set of
prime factors is exactly `x.primeFactors.erase q`: either the cofactor
`D = ∏ p ∈ x.primeFactors.erase q, p` itself works, or else `D < A` and the
power `D ^ A` does. -/
lemma erase_primeFactors_representative {A x q : ℕ} (hA : 1 < A) (hx : 1 < x)
...
/-- For any sequence satisfying the condition, there exist positive integers
`T` and `L` such that `a (n + T) = a n + L` for every `n`. Equivalently, the
sequence of consecutive differences is purely periodic. -/
problem imo2026_p6 (a : ℕ → ℕ) (ha : IsValidSeq a) :
    ∃ T L : ℕ, 0 < T ∧ 0 < L ∧ ∀ n, a (n + T) = a n + L := by
  -- Call an index `n` a *core* index when the support `(a n).primeFactors`
  -- contains no earlier support.
  let IsCore : ℕ → Prop := fun n =>
    ∀ i < n, ¬(a i).primeFactors ⊆ (a n).primeFactors
  -- Any two terms share a common factor.
  ...
  -- Every support contains a core support.
  ...
  -- A skipped value `b ∈ [a 0, a n)` is coprime to some earlier term.
  ...
  -- Every prime occurring in a core support is at most `(a 0) ^ (a 0 + 2)`:
  -- otherwise the representative from `erase_primeFactors_representative` is a
  -- skipped value, coprime to some earlier term, and induction applies.
  ...
  -- Hence there are only finitely many core indices.
  ...
  -- A `b` meeting every core term meets every term.
  ...
  -- Every `b ≥ a 0` meeting all terms is itself a term.
  have compatible_mem_range {b : ℕ}
    ...
    obtain ⟨i, hi, hicop⟩ := skipped_has_coprime hab hba (by
      ...
      exact hnot ⟨i, hai⟩)
    exact hb i hicop
  -- The shift `L`: the product of the (finitely many) core terms.
  ...
  -- The "meets every core term" property is periodic with period `L`.
  ...
  have hgood_all {b : ℕ} (hb : Good b) : ∀ i, ¬Nat.Coprime b (a i) :=
    compatible_all_of_core hb
  -- `a 0 + L` is a term, say `a T`; then shifting by `L` commutes with the
  -- enumeration, giving `a (n + T) = a n + L` for all `n`.
